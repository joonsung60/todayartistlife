...
응답 작성 시:
- topic은 한국어 문장으로 작성하세요. 인물명, 작품명, 행사명 등 고유명사는 영문 그대로 써도 됩니다. topic 전체가 영어 문장이 되는 것은 절대 금지입니다.
좋은 예: 'Taylor Swift, 결혼식 하객에게 NDA 요구' / 'Harry Styles, Wembley 공연에 Shania Twain 깜짝 등장'
나쁜 예: 'Taylor Swift Wedding NDA Requirements' / 'Harry Styles Wembley Residency'
...
// Supabase entities 테이블에서 name → weight 매핑을 로드한다.
// 실패해도 빈 Map을 반환해 weight 기능만 비활성화되고 전체 흐름은 유지된다.
async function loadEntityWeights(): Promise<Map<string, number>> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('name, weight')
    if (error) {
      console.error('[stage1] entities weight 로드 실패:', error.message)
      return weightMap
    }
    ...
    console.error('[stage1] entities weight 로드 예외:', err)
  }
  return weightMap
}

// 기사에 매칭된 엔티티들의 weight 합산 점수. weight 미존재 시 기본값 1.
function calcArticleScore(
  articleId: string,
  articleMatchMap: Map<string, string[]>,
  weightMap: Map<string, number>
): number {
  const matched = articleMatchMap.get(articleId) ?? []
  return matched.reduce((sum, name) => sum + (weightMap.get(name) ?? 1), 0)
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const entities = loadTargetEntities()
    const weightMap = await loadEntityWeights()
    ...
    // 매칭된 엔티티 weight 합산 점수가 높은 기사를 우선 LLM에 투입
    const scoreMap = new Map<string, number>(
      withEntities.map((a) => [a.id, calcArticleScore(a.id, articleMatchMap, weightMap)])
    )
    const llmInput = [...withEntities]
      .sort((a, b) => (scoreMap.get(b.id) ?? 0) - (scoreMap.get(a.id) ?? 0))
      .slice(0, LLM_INPUT_MAX)
    ...
    const top3 = llmInput
      .slice(0, 3)
      .map((a) => `${a.title} (score=${scoreMap.get(a.id) ?? 0})`)
    console.log(`[stage1] weight 상위 3 기사: ${top3.join(' | ')}`)
